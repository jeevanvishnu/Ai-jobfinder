...
import { Mail, Lock, Eye, EyeOff, User, ArrowRight, LoaderCircle } from 'lucide-react';
import { useDispatch, useSelector } from 'react-redux';
import toast from 'react-hot-toast';
import { clearAuthError, signupUser, startGoogleAuth } from '../features/AuthSlice';
...
const Signup = () => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const message =
        typeof submitError === 'string' ? submitError : 'Unable to create your account.';
      toast.error(message);
    }
  };
  ...
  return (
    <>
      <h2 className="text-2xl font-bold mb-2 text-gray-900 tracking-tight font-inter">Create an Account</h2>
      <p className="text-gray-500 text-[14px] mb-6">
        Sign up to get started with AutoJob AI
      </p>
      ...
      <form className="space-y-4" onSubmit={handleSubmit}>
        ...
        >
          {loading ? (
            ...
            <>
              Sign Up <ArrowRight className="ml-1.5 h-[18px] w-[18px]" strokeWidth={2} />
            </>
          )}
        </button>
      </form>
      ...
      <div className="mt-6 grid grid-cols-1 gap-3">
        <button onClick={handleGoogleSignup} disabled={loading} type="button" className="flex items-center justify-center px-4 py-2 border border-gray-200 rounded-lg text-[13px] font-bold text-gray-700 bg-white hover:bg-gray-50 hover:border-gray-300 transition-all shadow-sm h-10 disabled:cursor-not-allowed disabled:opacity-70">
